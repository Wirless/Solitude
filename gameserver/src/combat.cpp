...
ReturnValue Combat::canDoCombat(Creature* attacker, Creature* target)
{
	...
	if (g_game.getWorldType() == WORLD_TYPE_NO_PVP) {
		// Check if the attacker is a player or a summon with a player master
		if (attacker->getPlayer() || (attacker->isSummon() && attacker->getMaster()->getPlayer())) {
			if (target->getPlayer()) {
				// Prevent player vs player combat in non-PvP zones
			...
			if (target->isSummon()) {
				...
				if (targetMasterPlayer) {
					...
					// Prevent summon vs player combat in non-PvP zones if they aren't in the same party
					if (!isInPvpZone(attacker, target)) {
						return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
					}
				}
...
void MagicField::onStepInField(Creature* creature)
{
	...
	// Skip damage if field ID is 1500, 1501, 1502, 1503, or 1504 and creature is a player
	if ((id == 1500 || id == 1501 || id == 1502 || id == 1503 || id == 1504) && creature->getPlayer()) {
		return;
	}
